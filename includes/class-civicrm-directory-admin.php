...
class CiviCRM_Directory_Admin {
	...
	public function groups_get() {
		...
		if ( ! function_exists( 'civi_wp' ) ) return $groups;
		if ( ! civi_wp()->initialize() ) return $groups;
	 ...
	 *
	 *     (1) by first letter ($mode = 'all')
	 *     (2) by category ($mode = 'all')
	 *     (3) by name ($mode = 'all')
	 *     (4) by key word ($mode = 'all')
	 *
	 * @since 0.1
	 *
	 ...
	 */
	public function contacts_get_for_group( $group_id, $mode = 'all', $query = '' ) {
		...
		switch( $mode ) {
			case 'first_letter' :
				$params['organization_name'] = array( 'LIKE' => $query . '%' );
				break;
			case 'name' :
				$params['organization_name'] = array( 'LIKE' => '%' . $query . '%' );
				break;
